        ...
        <div className="flex gap-1.5 flex-shrink-0">
          {Object.entries(statusConfig).map(([status, cfg]) => {
            ...
            return (
              <button
                key={status}
                onClick={() => !isResubmit && onStatusChange(status)}
                disabled={isResubmit}
                className={cn(
                  ...
                  isResubmit && "cursor-not-allowed opacity-60"
                )}
                title={isResubmit ? "Resubmit (auto-set after 30 days)" : status.charAt(0).toUpperCase() + status.slice(1)}
              >
